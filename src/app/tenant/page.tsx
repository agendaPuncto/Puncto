...
export default function PublicBusinessPage() {
  ...
  const [activeTab, setActiveTab] = useState<'book' | 'menu' | 'table' | 'gallery' | 'reviews'>(
    isRestaurant ? 'menu' : 'book'
  );
  ...
  const tabs = isRestaurant ? restaurantTabs : serviceTabs;
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              {user ? (
                <>
                  <span className="text-stone-200 text-sm">Olá, {user.displayName || user.email}</span>
                  <button onClick={() => { logout(); router.refresh(); }} className="px-4 py-2 text-sm font-medium text-stone-200 hover:text-white hover:bg-white/10 rounded-lg transition-colors">Sair</button>
                </>
              ) : (
                <>
                  <a href="/auth/login" className="px-4 py-2 text-sm font-medium text-stone-200 hover:text-white transition-colors">Entrar</a>
                  <a href="/auth/signup" className="px-4 py-2 text-sm font-medium bg-white text-stone-900 rounded-lg hover:bg-stone-100 transition-colors">Criar conta</a>
                </>
              )}
            </div>
          </div>
