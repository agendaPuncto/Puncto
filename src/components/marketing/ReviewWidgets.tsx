'use client';

import { motion } from 'framer-motion';

// Google Reviews Widget
interface GoogleReviewsProps {
  placeId?: string;
  rating: number;
  reviewCount: number;
  className?: string;
}

export function GoogleReviewsBadge({
  placeId,
  rating = 4.9,
  reviewCount = 127,
  className = '',
}: GoogleReviewsProps) {
  const stars = Array.from({ length: 5 }, (_, i) => i < Math.floor(rating));
  const hasHalfStar = rating % 1 >= 0.5;

  return (
    <a
      href={placeId ? `https://search.google.com/local/reviews?placeid=${placeId}` : '#'}
      target="_blank"
      rel="noopener noreferrer"
      className={`inline-flex items-center gap-3 bg-white rounded-xl px-4 py-3 border border-slate-200 hover:border-primary-200 hover:shadow-soft transition-all ${className}`}
    >
      {/* Google logo */}
      <svg className="w-6 h-6" viewBox="0 0 24 24">
        <path
          fill="#4285F4"
          d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        />
        <path
          fill="#34A853"
          d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        />
        <path
          fill="#FBBC05"
          d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        />
        <path
          fill="#EA4335"
          d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        />
      </svg>

      <div className="flex flex-col">
        <div className="flex items-center gap-1">
          <span className="font-bold text-slate-900">{rating.toFixed(1)}</span>
          <div className="flex">
            {stars.map((filled, i) => (
              <svg
                key={i}
                className={`w-4 h-4 ${filled ? 'text-yellow-400' : 'text-slate-200'}`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
            ))}
          </div>
        </div>
        <span className="text-xs text-slate-500">{reviewCount} avaliações no Google</span>
      </div>
    </a>
  );
}

// Trustpilot Widget
interface TrustpilotProps {
  businessUnitId?: string;
  rating: number;
  reviewCount: number;
  className?: string;
}

export function TrustpilotBadge({
  businessUnitId,
  rating = 4.8,
  reviewCount = 89,
  className = '',
}: TrustpilotProps) {
  const getTrustScore = (r: number) => {
    if (r >= 4.5) return 'Excelente';
    if (r >= 4) return 'Ótimo';
    if (r >= 3) return 'Bom';
    return 'Regular';
  };

  return (
    <a
      href={businessUnitId ? `https://www.trustpilot.com/review/${businessUnitId}` : '#'}
      target="_blank"
      rel="noopener noreferrer"
      className={`inline-flex items-center gap-3 bg-white rounded-xl px-4 py-3 border border-slate-200 hover:border-primary-200 hover:shadow-soft transition-all ${className}`}
    >
      {/* Trustpilot logo */}
      <svg className="w-24 h-6" viewBox="0 0 126 31">
        <path
          fill="#00B67A"
          d="M30.141 11.07H18.63L15.07 0l-3.558 11.07H0l9.337 6.791-3.558 11.07 9.291-6.79 9.337 6.79-3.558-11.07 9.292-6.791z"
        />
        <path
          fill="#005128"
          d="M21.616 20.107l-.792-2.446-5.754 4.2 6.546-1.754z"
        />
      </svg>

      <div className="flex flex-col">
        <div className="flex items-center gap-2">
          <span className="font-bold text-slate-900">{getTrustScore(rating)}</span>
          <div className="flex gap-0.5">
            {[1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className={`w-5 h-5 ${i <= Math.round(rating) ? 'bg-[#00B67A]' : 'bg-slate-200'}`}
              >
                <svg className="w-full h-full text-white p-0.5" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              </div>
            ))}
          </div>
        </div>
        <span className="text-xs text-slate-500">
          Com base em {reviewCount} avaliações
        </span>
      </div>
    </a>
  );
}

// Combined Reviews Section
interface ReviewsSectionProps {
  googleRating?: number;
  googleReviews?: number;
  trustpilotRating?: number;
  trustpilotReviews?: number;
}

export function ReviewsSection({
  googleRating = 4.9,
  googleReviews = 127,
  trustpilotRating = 4.8,
  trustpilotReviews = 89,
}: ReviewsSectionProps) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      className="flex flex-wrap items-center justify-center gap-4"
    >
      <GoogleReviewsBadge rating={googleRating} reviewCount={googleReviews} />
      <TrustpilotBadge rating={trustpilotRating} reviewCount={trustpilotReviews} />
    </motion.div>
  );
}

// Individual Review Card
interface ReviewCardProps {
  author: string;
  rating: number;
  content: string;
  date: string;
  source: 'google' | 'trustpilot';
  avatar?: string;
}

export function ReviewCard({
  author,
  rating,
  content,
  date,
  source,
  avatar,
}: ReviewCardProps) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      className="bg-white rounded-xl p-6 border border-slate-200 hover:shadow-soft transition-shadow"
    >
      {/* Header */}
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          {avatar ? (
            <img src={avatar} alt={author} className="w-10 h-10 rounded-full" />
          ) : (
            <div className="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center">
              <span className="font-semibold text-primary-600">
                {author.charAt(0)}
              </span>
            </div>
          )}
          <div>
            <p className="font-medium text-slate-900">{author}</p>
            <p className="text-xs text-slate-500">{date}</p>
          </div>
        </div>

        {/* Source badge */}
        {source === 'google' ? (
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
        ) : (
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path fill="#00B67A" d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" />
          </svg>
        )}
      </div>

      {/* Stars */}
      <div className="flex gap-0.5 mb-3">
        {[1, 2, 3, 4, 5].map((i) => (
          <svg
            key={i}
            className={`w-4 h-4 ${i <= rating ? 'text-yellow-400' : 'text-slate-200'}`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        ))}
      </div>

      {/* Content */}
      <p className="text-slate-600 text-sm line-clamp-4">{content}</p>
    </motion.div>
  );
}

// Aggregate Rating Schema for SEO
export function AggregateRatingSchema({
  ratingValue,
  reviewCount,
  bestRating = 5,
  worstRating = 1,
}: {
  ratingValue: number;
  reviewCount: number;
  bestRating?: number;
  worstRating?: number;
}) {
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'AggregateRating',
    ratingValue: ratingValue.toString(),
    reviewCount: reviewCount.toString(),
    bestRating: bestRating.toString(),
    worstRating: worstRating.toString(),
  };

  return (
    <script
      type="application/ld+json"
      dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }}
    />
  );
}
